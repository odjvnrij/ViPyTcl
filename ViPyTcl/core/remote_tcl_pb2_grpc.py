...
class RemoteTclServicer(object):
    """我们rpc服务的名字
    后面
    服务端 会用到 <ComputeServicer>
    客户端 会用到 <ComputeStub>
...
def add_RemoteTclServicer_to_server(servicer, server):
    rpc_method_handlers = {
    ...
    generic_handler = grpc.method_handlers_generic_handler(
            'remote_tcl.RemoteTcl', rpc_method_handlers)
    server.add_generic_rpc_handlers((generic_handler,))
    server.add_registered_method_handlers('remote_tcl.RemoteTcl', rpc_method_handlers)


 # This class is part of an EXPERIMENTAL API.
class RemoteTcl(object):
    """我们rpc服务的名字
    后面
    服务端 会用到 <ComputeServicer>
    客户端 会用到 <ComputeStub>
